...
```
    ...
    主循环处理 <------ 更新时间和URL <--- 定时触发
        ↓
    检测按键 ---> 语音录制 ---> WebSocket通讯(ASR)
        ↓                       ↓
    语音处理 ---> 语音播放 <--- WebSocket通讯(NLP)
        ↓
    OLED状态显示
```
...
目前还是一个注册加回调函数的方式，屏幕显示不能显示一些动态的表情之类的。现在的代码就挺屎山的，所以后面打算重构一下并引入FreeRTOS。
...
String APPID = "508482c0"; 
String APIKey = "533a93f2e37fcdecc92e46421aee339a";
String APISecret = "ZTIwOTMwM2JlOWIxZTMxNDM5NGEzN2Uy";
...
int noise = 50;
HTTPClient https;
U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, /* reset=*/ U8X8_PIN_NONE, /* clock=*/ 12, /* data=*/ 11); // OLED
...
int loopcount = 0;
void onMessageCallback(WebsocketsMessage message) //NLP 消息回调函数
{
...
void onEventsCallback(WebsocketsEvent event, String data) //NLP 事件回调函数
{
...
void onMessageCallback1(WebsocketsMessage message) //语音流转文本 消息回调函数
{
    StaticJsonDocument<4096> jsonDocument;
    DeserializationError error = deserializeJson(jsonDocument, message.data());
...
void onEventsCallback1(WebsocketsEvent event, String data)
{
    if (event == WebsocketsEvent::ConnectionOpened)
    {
        ...
        while (1)
        {
            ...
            audio1.Record();
            float rms = calculateRMS((uint8_t *)audio1.wavData[0], 1280);//i2s数据存入wavedata了
            printf("%d %f\n", 0, rms);
            if (rms < noise)
            {
                if (voicebegin == 1)
            ...
            if (silence == 6)
            {
                ...
                u8g2.drawStr(0,10,"Thinking...");  // 写文字
                u8g2.drawStr(0,20,"OvO");
                u8g2.sendBuffer();
    ...
    for (int i = 0; i < numNetworks; ++i)
    {
        ...
        while (WiFi.status() != WL_CONNECTED)
        {
            ...
            if (count >= 30)
            {
                Serial.printf("\r\n-- wifi connect fail! --");
                u8g2.clearBuffer();         // 清空缓存
                u8g2.drawStr(0,10,"^^");  // 写文字
                u8g2.drawStr(0,30,"Wifi connection failed.");  // 写文字
                u8g2.drawStr(0,40,"Please restart");  // 写文字
                u8g2.drawStr(0,50,"and try again.");
                u8g2.sendBuffer();
                break;
            }
            vTaskDelay(100);
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            return; // 如果连接成功，退出函数
            u8g2.clearBuffer();         // 清空缓存
            u8g2.drawStr(0,10,"Wifi Connected");  // 写文字
            u8g2.sendBuffer();
        }
    }
}
...
void setup()
{
    ...
    u8g2.begin();
    u8g2.clearBuffer();         // 清空缓存
    u8g2.setFont(u8g2_font_HelvetiPixel_tr); // 选择字体
    u8g2.drawStr(0,10,"ChatBot Init");  // 写文字
    u8g2.sendBuffer();
    // pinMode(ADC,ANALOG);
    ...
    audio2.setVolume(50);
    audio2.connecttospeech("Hello! I'm Iris,your personal AI assistant", "en");
...
int flagfirsttalking = 0;//全局变量，记录是否在说话
void loop()
{
    ...
    {
        if(flagfirsttalking == 0){
            u8g2.clearBuffer();         // 清空缓存
            u8g2.drawStr(0,10,"Talking...");  // 写文字
            u8g2.sendBuffer();
            flagfirsttalking = 1;
        }
    }
    ...
    {
        if(flagfirsttalking == 1){
            u8g2.clearBuffer();         // 清空缓存
            u8g2.setFont(u8g2_font_helvB24_tr);
            u8g2.drawStr(0,30,"^^");  // 写文字
            u8g2.sendBuffer();
        ...
        //digitalWrite(led3, LOW);
        if ((urlTime + 240000 < millis()) && (audio2.isplaying == 0)) //鉴权
        {
            urlTime = millis();
            getTimeFromServer();
            url = getUrl("ws://spark-api.xf-yun.com/v3.5/chat", "spark-api.xf-yun.com", "/v3.5/chat", Date);
    ...
    if (digitalRead(key) == 0)
    {
        ...
        u8g2.clearBuffer();         // 清空缓存
        u8g2.drawStr(0,10,"Listening...");  // 写文字
        u8g2.sendBuffer();
